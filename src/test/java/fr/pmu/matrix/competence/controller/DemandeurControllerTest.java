...
public class DemandeurControllerTest {
    ...
    @InjectMocks
    private DemandeurController demandeurController;

    @BeforeEach
    public void setup() {
        MockitoAnnotations.initMocks(this);
        mockMvc = MockMvcBuilders.standaloneSetup(demandeurController).build();
    }
    ...
    public void testGetPersonnesDisponibles() throws Exception {
        ...
        when(demandeurService.getPersonnesDisponibles()).thenReturn(profils);

        // Exécution et vérification
        mockMvc.perform(get("/demandeurs/personnes-disponibles")
    ...
    public void testGetMatriceCompetencePersonne() throws Exception {
        ...
        // Configuration du mock
        when(demandeurService.getMatriceCompetencePersonneDisponible("P001")).thenReturn(matriceCompetences);

        // Exécution et vérification
        mockMvc.perform(get("/demandeurs/personnes-disponibles/P001/competences")
                .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$", hasSize(2)))
    ...
    public void testFiltrerPersonnesParNotes() throws Exception {
        ...
        // Configuration du mock
        when(demandeurService.filtrerPersonnesParNotes(anyList(), anyInt())).thenReturn(personnes);

        // Exécution et vérification
        mockMvc.perform(get("/demandeurs/filtrer-par-notes")
    ...
    public void testFiltrerPersonnesParCompetences() throws Exception {
        ...
        // Configuration du mock
        when(demandeurService.filtrerPersonnesParCompetences(anyList())).thenReturn(personnes);

        // Exécution et vérification
        mockMvc.perform(get("/demandeurs/filtrer-par-competences")
